...
class Line():
    ...
    def add_fit(self, limg, rimg, mx=3.75 / 900, my=20 / 700):
        ...
        # Otherwise, Calculate the parameters. 
            ...
            self.lcurrent_fit = np.polyfit(ly_points * my, lx_points * mx,
                                           2)  # pndarray, shape (deg + 1,) or (deg + 1, K)
            # Calculate the y value of the base point

            # self.ly_base = my*np.float(self.limg.shape[0])
            # self.ly_base = my * np.float64(self.limg.shape[0])
            self.ly_base = my * float(self.limg.shape[
                                          0])  ### np.float` was a deprecated alias for the builtin `float`. To avoid this error in existing code, use `float` by itself.

            ly_base = my * self.limg.shape[0]
            # Calculate the radius of the curvature of the lane. Note that I didn't use 'np.abs' to get the absolute
            # the value.
            self.lradius_of_curvature = ((1 + (
                    2 * self.lcurrent_fit[0] * ly_base + self.lcurrent_fit[1]) ** 2) ** 1.5) / (
                                                2 * self.lcurrent_fit[0])
            # Calculate the horizontal location of the base point
            self.lline_base_pos = self.lcurrent_fit[0] * ly_base ** 2 + self.lcurrent_fit[1] * ly_base ** 1 + \
                                  self.lcurrent_fit[2] * ly_base ** 0 - mx * float(self.limg.shape[1]) / 2
            ...
            self.ry_base = my * float(self.rimg.shape[0])

            ry_base = my * self.rimg.shape[0]
            self.rradius_of_curvature = ((1 + (
                    2 * self.rcurrent_fit[0] * ry_base + self.rcurrent_fit[1]) ** 2) ** 1.5) / (
                                                2 * self.rcurrent_fit[0])

            self.rline_base_pos = self.rcurrent_fit[0] * ry_base ** 2 + self.rcurrent_fit[1] * ry_base ** 1 + \
                                  self.rcurrent_fit[2] * ry_base ** 0 - mx * float(self.rimg.shape[1]) / 2
    ...
    def best_fit(self, mx=3.75 / 900, my=20 / 700):
        ...
        if 1 / (self.rradius_of_curvature * self.rradius_of_curvature) > 1 / (-1000000):
            if np.abs(self.rradius_of_curvature - self.lradius_of_curvature) / (
                    self.rradius_of_curvature + self.lradius_of_curvature) < 0.3:
        ...
        self.lradius_of_curvature = ((1 + (2 * self.lfit[0] * self.ly_base + self.lfit[1]) ** 2) ** 1.5) / (
                2 * self.lfit[0])

        self.lline_base_pos = self.lfit[0] * self.ly_base ** 2 + self.lfit[1] * self.ly_base ** 1 + self.lfit[
            2] * self.ly_base ** 0 - mx * float(self.limg.shape[1]) / 2

        self.rradius_of_curvature = ((1 + (2 * self.rfit[0] * self.ry_base + self.rfit[1]) ** 2) ** 1.5) / (
                2 * self.rfit[0])

        self.rline_base_pos = self.rfit[0] * self.ry_base ** 2 + self.rfit[1] * self.ry_base ** 1 + self.rfit[
            2] * self.ry_base ** 0 - mx * float(self.rimg.shape[1]) / 2
...
def threshold_binary_lane(img):
    warp_img = warp(img)
    thresh = combined_threshold(warp_img)
    rgb_image = np.dstack((thresh, thresh, thresh)) * 255  # convert the numpy array back to three channel RGB image
...
video_output1 = './examples/project_video_binary_lane.mp4'
video_input1 = VideoFileClip('project_video.mp4')
# processed_video = video_input1.fl_image(process_img) # apply functions to each frame of the video
processed_video = video_input1.fl_image(threshold_binary_lane)  # apply functions to each frame of the video
# processed_video = video_input1.fl_image(warp)  # generate bird-eye view video
